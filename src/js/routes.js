
const routes = {
    indexRoute : '/',
    aboutRoute : '/about',
    committeMembers : '/commitee_members',
    templeSevaRoute : '/temple_seva',
    kumaraSwamySevaRoute : '/kumara_swamy_seva',
    pictureGalleryRoute : '/picture_gallery',
    videoGalleryRoute : '/video_gallery',
    malaDharanaRegistrationRoute : '/mala_dharana_registration',
    irumudiYatraRegistrationRoute : '/irumudi_yatra_resgistration',
    housePoojaRequestRoute : '/house_pooja_request',
    ayyappaDeekshaRoute : '/ayyappa_deeksha',
    guideLinesRoute : '/guidelines',
    ayyappaDeekshaTitlesRoute : '/ayyappa_deeksha_titles',
    poojaSponsorsRoute : '/pooja_sponsors',
    useFullLinksRoute : '/use_full_links',
    contactRoute : '/contact',
    donateRoute : '/donate',
    logInRoute : '/login',
    termsAndConditionsRoute : '/terms_conditions',
    donationPolicyRoute : '/donation_policy',
    privacyAndDisclaimerRoute : '/privacy_disclaimer',
    faqRoute : '/faq',
}

export default routes;